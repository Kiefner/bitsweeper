...
$(function(){
    ...
    if(getURLParameter('access_token')!="" && getURLParameter('access_token')!=null){
        ...
        window.history.pushState('', 'Coinmines', '/');
    }
    ...
    socket.on('connect', function() {
        ...
        socket.emit('auth', authRequest, function(err, authResponse) {
            ...
            if(access_token!="" && access_token!=null){
                ...
                $.getJSON("https://api.moneypot.com/v1/list-bets?access_token="+access_token+"&app_id=1192&wager=10000000&limit=11&order_by=desc", function(json){
    ...
    socket.on('new_bet', function(payload) {
        ...
        cell3.innerHTML = (win?"<span style=\"color: #8D4\">"+(payload.profit/100).formatMoney(2, '.', ',')+"</span>"+" Bits":"<span style=\"color: #E74C3C\">"+(payload.profit/100).formatMoney(2, '.', ',')+"</span>"+" Bits");
...
$('#moneypotDepositButton').click(function() {
    var windowUrl = 'https://www.moneypot.com/dialog/deposit?app_id=1192';
...
$('#withdrawButton').click(function() {
    var windowUrl = 'https://www.moneypot.com/dialog/withdraw?app_id=1192';
    var windowName = 'manage-auth';
    var windowOpts = 'width=420,height=350,left=100,top=100';
    var windowRef = window.open(windowUrl, windowName, windowOpts);
    windowRef.focus();
...
function onloadCallback() {
    grecaptcha.render('faucetClaimCaptcha', {
        'sitekey' : '6LcnKSETAAAAAEvBjdmeusfq5H6_Y5siHOSLUGBd',
...
function startNewGame(){
    if(!connected){
        $('#theDimmer').html("Error: Not connected!<br><a href=\"https://www.moneypot.com/oauth/authorize?app_id=1192&response_type=token&state=Meh&redirect_uri=http://kiefner.github.io/coinmines/\">Click here to login</a>");
